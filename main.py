...
def load_data():
    data = data_reader.CowCube().normal_cube
    data_gt_mixed = data_reader.CowCube().truth_mixed
    # data_gt = data_reader.CowCube().truth_ed
    data_gt = data_reader.CowCube().truth_ml

    return data, data_gt, data_gt_mixed

# def load_data():
#     data = data_reader.PigletCube().normal_cube
#     data_gt = data_reader.PigletCube().truth

#     return data, data_gt


# def load_data():
#     data = data_reader.HumanCube().normal_cube
#     data_gt = data_reader.HumanCube().truth

#     return data, data_gt

data, data_gt, data_gt_mixed = load_data()

class_num = np.max(data_gt)
height, width, bands = data.shape
gt_reshape = np.reshape(data_gt, [-1])
gt_reshape_mixed = np.reshape(data_gt_mixed, [-1])
# load config
...
# split datasets
# _ , _, test_index = split_data.split_data(gt_reshape, 
...
# print("inital: ", type(train_index),train_index.shape )
...
train_size = (int)(np.count_nonzero(data_gt) * split_size // class_count)
print("train_size:", train_size)

train_index = []
val_index = []
# test_samples_gt = np.copy(gt_reshape_mixed)


for c in np.unique(gt_reshape):
    if c == 0:
        continue
    indices = np.nonzero(gt_reshape == c)
    X = list(zip(*indices))  # x,y features

    train_index += random.sample(X, train_size)


for c in np.unique(gt_reshape):
    if c == 0:
        continue
    val_indices = np.nonzero(gt_reshape == c)

    val_ind = []

    for v_l in val_indices[0]:
        if v_l not in X: # meaning harc?
            val_ind.append(v_l)

        X_val = list(zip(*val_indices))  # x,y features

        val_index += random.sample(X_val, train_size)
...
all_indices = np.arange(gt_reshape_mixed.shape[0]) 
train_val_index = np.concatenate((train_index, val_index))  # Combine train and val indexes
test_index = np.setdiff1d(all_indices, train_val_index)
...
# create graph
# train_samples_gt, _, val_samples_gt = create_graph.get_label(gt_reshape,
#                                                  train_index, val_index, test_index)


# _, test_samples_gt, _ = create_graph.get_label(gt_reshape,
#                                                  train_index, val_index, test_index)
...
# saving gt
np.save("HSI_Classification/ZZ_WFCG/output/test_samples_gt", test_samples_gt.numpy())
...
# saving predic
np.save("HSI_Classification/ZZ_WFCG/output/predic", predict)
